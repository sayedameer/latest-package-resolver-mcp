---
name: bun-latest-package-resolver
...
Run the MCP server directly via `bunx`:

```bash
bunx @sayedameer/latest-package-resolver-mcp
```
...
```bash
echo '{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"get_latest_version","arguments":{"package":"next"}}}' | bunx @sayedameer/latest-package-resolver-mcp
```
